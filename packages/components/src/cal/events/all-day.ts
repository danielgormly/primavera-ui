...
export class AllDayEvents {
  ...
  updateRowCount(count: number) {
    ...
    this.cal.container?.style.setProperty("--rows", this.rows.toFixed());
  }
  ...
  renderExpanded(layout: ExpandedEventLayoutSet) {
    ...
    layout.forEach((lane, laneIndex) => {
      lane.forEach((event) => {
        const x = this.cal.transform.dateToX(event.startZero);
        ...
        div.style.width = `${this.cal.transform.dayPx * event.durationDays - 3}px`; // TODO: We need to actually vary it!
  ...
  renderContracted(
    ...
    const divs = events.map((event) => {
      const x = this.cal.transform.dateToX(
        utcZeroDate(event.start).valueOf(),
      );
      const div = document.createElement("div");
      div.classList.add("all-day-event", `col_${event.color}`);
      div.style.transform = `translate(${x}px)`;
      div.style.width = `${this.cal.transform.dayPx * event.dayLength - 3}px`; // TODO: We need to actually vary it!
      div.innerText = event.title;
      return div;
    });
